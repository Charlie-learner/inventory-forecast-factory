...
本文说明哪些内容属于源码、固定示例、本地数据或可删除产物，避免把临时运行结果和业务数据混入核心代码。
...
| `scripts/` | 数据生成、组合用例、外部服务测试、项目自检和安全清理脚本 | 是 |
| `docs/` | 架构、接口、业务口径、功能追踪和使用文档 | 是 |
...
`examples/` 与 `artifacts/` 的区别是：前者是经过筛选、适合版本化的稳定示例；后者是每次运行产生的本地状态，不应提交。
...
├── services/        回测、补货、性能、版本和项目自检等应用服务
