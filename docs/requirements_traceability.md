...
本表用于记录项目功能及其实现边界，区分“已经有代码和测试的功能”“只完成一部分的功能”和“目前没有实现的功能”。

> 需要整体检查项目时，可以先执行 `uv run python -m inventory_agent audit --strict`，并查看
> [`evaluation_acceptance_matrix.md`](evaluation_acceptance_matrix.md)。该矩阵由 CLI 与 Web
> “项目自检”页面共用的文件检查器生成；本文保留更详细的实现边界说明。

- **完成**：仓库中有实现、可运行入口和自动化测试或示例证据。
- **部分完成**：主链路可用，但与预期设计相比仍有明确边界。
...
| 验证报告样例 | 完成 | `examples/complete_run/validation_report.{json,md}`；`examples/detailed_run/` 含完整事件流；`examples/repair_run/` 展示首次失败、修复和再验证 | 样例由示例生成脚本维护 |
| README | 完成 | `README.md` 覆盖题目要求的十个章节 | 无 |
| 环境依赖文件 | 完成 | `pyproject.toml`、`uv.lock`、`requirements.txt` | 无 |
| 使用示例和测试用例 | 完成 | `docs/usage_examples_and_test_cases.md` 提供 Web/CLI 示例和 24 项测试清单；离线自动测试覆盖六类需求画像、多智能体代码协作、任务持久化与核心流程；外部服务脚本实际检查 LLM 和 Crossref | 单次批量完整工作流限制为 20 个商品—仓库组合；外部服务测试依赖网络 |
...
```bash
uv run python -m inventory_agent --help
uv run python -m inventory_agent extract-capability --source . --output knowledge/extracted_capabilities.json
uv run python -m inventory_agent replicate-capability --source examples/capabilities/moving_average.md --manifest artifacts/replication/review_manifest.json
uv run python -m inventory_agent versions list --knowledge artifacts/knowledge/capability_graph.json --model moving_average
uv run python -m inventory_agent run --description "为商品 3424 在仓库 1 预测未来14天目标库存" --data examples/data/cainiao_demo.csv --capability-source examples/capabilities/moving_average.md
uv run python -m inventory_agent visualize-graph --output artifacts/knowledge/capability_graph.html
...
以上状态只描述当前仓库中已经有代码、测试或运行结果的能力，不把“以后可以扩展”写成“现在已经实现”。
# 扩展功能补充说明

扩展功能的运行结果、改进内容和边界已整理到
