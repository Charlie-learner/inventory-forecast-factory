...
本目录保存适合放进 Git 的小型示例和运行结果。日常 Web 上传和临时运行结果位于 `artifacts/`，不会进入版本库。
...
| `repair_run/` | 首次失败、根因分析、自动修复和重新验证 | `README.md` 与 `index.json` 指向当前精选运行 |
| `advanced_showcase/` | 联网研究、多智能体代码、经验复用、性能和图谱写回的综合展示 | `README.md` 指向最新离线证据和最近真实联网证据 |
| `acceptance/` | 多场景自动测试汇总 | Markdown 与 JSON 报告 |
| `external_validation/` | 已脱敏的外部 LLM/Crossref 实测摘要 | 不包含 API Key |
...
其中 `advanced_showcase/` 是综合示例入口；其余目录用于单独查看某项功能。更新这些示例前应先运行自动测试，确认结果正常后再替换对应目录，不要直接把整个 `artifacts/` 复制进来。
